...
use_pretrain=True
###########
WANDB_ENABLE = True  # enable wandb upload
...
def get_wandb_run(dataname, env_number):
    ...
    project = os.getenv("WANDB_PROJECT", "crosscity-stgcn-disturb")
    entity = os.getenv("WANDB_ENTITY", None)
    wandb_run = wandb.init(
        project=project,
        entity=entity,
        name=f"{dataname}_env{env_number}",
        config={"dataname": dataname, "model": "v_STGCN5", "use_pretrain": use_pretrain},
    )
    # Unify step metric to avoid regression across stages/graphs
...
def load_params(model,param):
    ...
    return model
@torch.no_grad()
def disturbe_params(model,std_params,disturbe_rate=0.01,disturbe_number=0):
...
def reconstruct_series(windows: np.ndarray, horizon: int, stride: int = 1) -> np.ndarray:
    """
    Rebuild a full time series from sliding windows by averaging overlaps.
...
def make_graph_key(graph_index):
    if hasattr(graph_index, "__len__"):
        return tuple(float(x) for x in graph_index)
    return float(graph_index)
class Task4Trainer:
    ...
    def _evaluate_and_get_rmse(self, model, data_loader, A_wave):
        """Evaluate the model on a given dataset and return RMSE, predictions, and labels."""
        model.eval()
        all_outputs = []
        all_labels = []
        with torch.no_grad():
        ...
        rmse = torch.sqrt(self.loss_criterion(outputs, labels))
        
        return rmse.item(), outputs, labels

    def _get_env_serial(self, graph_index):
        key = make_graph_key(graph_index)
    ...
    def train_single_graph(self,
        ...
        if os.path.exists(model_path):
            self.logger.info(f"Found existing model for graph {graph_index} at {model_path}, skipping training")
            with torch.no_grad():
            ...
            self.logger.info(f"Graph {graph_index}: RMSE (loaded model, denorm) = {final_rmse:.6f}")

            # wandb only uploads evaluation before disturbance and after training
            if WANDB_ENABLE:
                ...
                run.log({**payload, "global_step": step}, step=step)
                print(f"[wandb] Upload success: env={env_number}, graph={graph_index}, step={step}")
                if env_serial is None:
                    wandb_steps[env_number] = step + 1

            return outputs, labels, final_rmse
        
        # If model weights do not exist, perform normal training
        ...
        if use_pretrain:
            rootpath=f'Param/stgcn/{self.dataname}/disturbe/'
        else:
            rootpath=f'Param/stgcn/{self.dataname}/disturbe_no_pretrain/'
        ...
        # Evaluate on test set before training (no training)
        initial_rmse, _, _ = self._evaluate_rmse_denorm(model, test_loader, A_wave, test_meanstd, return_outputs=False)
        self.logger.info(f"Graph {graph_index}: RMSE before training (denorm) = {initial_rmse:.6f}")
        # Save to disk immediately to avoid loss on exit
        ...
        with open(rmse_log_path, "a") as f:
            f.write(f"env={env_number}, graph={graph_index}, stage=before, rmse={initial_rmse:.6f}\n")
            f.flush()
        print(f"[RMSE] env={env_number}, graph={graph_index}, stage=before, rmse={initial_rmse:.6f}")
        ...
        for epoch in tqdm(range(self.epochs)):
            ...
            # wandb logging moved to upload together after each graph training is completed

            # Only record the best model when validation set is better
            if val_loss < best_loss:
                best_loss = val_loss
                # Move the state dictionary to CPU before deep copying to avoid GPU memory spikes
        ...
        # Final evaluation: Return denormalized RMSE, and keep output for subsequent summary
        final_rmse, outputs, labels = self._evaluate_rmse_denorm(
            model, test_loader, A_wave, test_meanstd, return_outputs=True
        )
    ...
    def _evaluate_rmse_denorm(self, model, data_loader, A_wave, meanstd, return_outputs=False):
        """Calculate denormalized RMSE on specified loader, optionally return outputs and labels"""
        model.eval()
        outputs_list = []
        labels_list = []
        with torch.no_grad():
        ...
        mean_val = torch.tensor(meanstd[0], device=outputs.device, dtype=outputs.dtype).reshape(1, -1, 1)
        std_val = torch.tensor(meanstd[1], device=outputs.device, dtype=outputs.dtype).reshape(1, -1, 1)
        outputs_denorm = outputs * std_val + mean_val
        labels_denorm = labels * std_val + mean_val
        rmse = torch.sqrt(torch.mean((outputs_denorm - labels_denorm) ** 2)).item()
...
if __name__ == '__main__':
    print("-"*15)
    print("Note that std only has one layer left")
    time.sleep(3)
    ...
    if args.taskmode == 'task4':
        ...
        if args.graph_range == 'A':
            graph_set=graph_set[:200]
            logger.info(f"Processing [0:200] graphs")
        elif args.graph_range == 'B':
            graph_set=graph_set[200:400]
            logger.info(f"Processing [200:400] graphs")
        else:  # args.graph_range == 'all'
            graph_set=graph_set
            logger.info(f"Processing all graphs")
        ...
        for idx, graph_index in enumerate(worker_graph_set):
            env_order_map[make_graph_key(graph_index)] = idx
        ...
        for env_number in env_range:
            for graph_index in worker_graph_set:
                ...
                if dataname == 'hill':
                    ...
                    trajectory_file = os.path.join("./graph_generator/output/fhn2/",
                                    f"undisturbed_trajectories_R_{graph_index[0]}_B_{graph_index[1]}.csv")
                    trajectory_file_x2 = os.path.join("./graph_generator/output/fhn2/",
                ...
                if use_pretrain:
                    ...
                    # Add disturbance
                    model_ori=disturbe_params(model_ori,std_params,disturbe_number=env_number)
                    set_seed(env_number)
                else:
                    rmse_pretrain = None
        ...
        if WANDB_ENABLE and wandb_runs:
            for run in wandb_runs.values():
                run.finish()
            print("[wandb] All runs completed and sync triggered")
